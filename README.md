...
## 对于上文的更正和补充
1. 在 main.c 中的在调用sd_read和sd_write等函数时，标准的做法应该是使用跳转表定义的函数指针来调用相应的BIOS函数，比如bios_sd_read、bios_sd_write等。
如果直接使用在目前的project下可能时没有问题的，但是在以后的task中可能会出现问题，因此建议使用跳转表函数指针来调用。
2. 在 main.c 中的键盘输入处理部分，建议添加对退格键的处理，这样用户在输入命令时可以进行简单的编辑操作。
3. 在 上文中的task5中，处理函数间数据传输时，我们建议在跳转表中添加相应的函数指针来实现数据传输功能，而不是直接操作内存地址。因为在后续的任务中，当加入虚拟内存时
每个任务的内存地址空间可能会发生变化，直接操作内存地址可能会导致错误。而且，我们可以举一个形象的例子，比如：
```bash
echo "task1 task2 task3" | cat > batch.txt
```
这里的管道符号`|`就实现了两个程序间的数据传输功能。
而我们知道echo这个程序其实就是getchar和putchar的组合，他是不涉及内存地址的。更进一步说，我们可以这样理解，对于一个用户程序，他自己是不知道自己的
输出是会被输出到标准输出还是被重定向到一个文件的，这些都是由操作系统来决定的。所以我们应该设计一个自己的数据传输函数，比如bios_transfer_data，这个函数可以在跳转表中定义，然后由内核来实现具体的功能，由内核决定数据的去向。(不过笔者在这个项目中偷懒没有修改这个事情 (｡•ᴗ-｡)♡ )

4. 在各个project中，A core是下一个S core任务的基础，但是S core任务可能会在后续的project中产生问题，请读者注意这一点。
